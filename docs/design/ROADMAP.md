...
<!-- roadmap-public: blurb="A drift-elimination program on main: schema, facts, and architecture generated and gated, plus a Kani-verified proof of the lexical path-confinement policy." -->
...
<!-- roadmap-public: blurb="alint explain surfaces a rule's full detail with a docs link and a one-line summary, rules show and list --search browse the catalog offline, and scope_filter gains manifest-derived path scoping so a rule tracks the manifest that declares its members instead of a drifting hand-maintained glob." -->
...
## Future / backlog
<!-- roadmap-public: blurb="A wasm plugin kind on a wasmtime host with a stable WIT interface, a filesystem sandbox, and a signed plugin registry, plus a language-agnostic duplicate-code detector, all unpinned from a numbered release until the engine work is scheduled." -->
