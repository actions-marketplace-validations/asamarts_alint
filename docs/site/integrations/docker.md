...
Tags published per release: the exact git tag (`:v0.9.21`), the bare semver (`:0.9.21`), the `<major>.<minor>` channel (`:0.9`), and `:latest`.
...
script:
  - docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.9.21 check --format json
```
