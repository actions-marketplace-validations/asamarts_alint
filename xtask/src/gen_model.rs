...
//! `alint.c4` crate elements against the `cargo metadata` member set, the
//! `config-model.c4` Config keys against the JSON schema, the taxonomy's
//! completeness against `all_kinds.yaml`, and every embedded `view-id` against
//! the model. (The crate-to-crate edges are generated by `gen-arch` into
//! `crate-graph.gen.c4`, so they're not gated here.)
...
fn model_view_ids(root: &Path) -> Result<BTreeSet<String>> {
    ...
    // Every `.c4` in the model dir (hand-authored + generated fragments like
    // rule-families.gen.c4 and crate-graph.gen.c4), so a generated view is seen.
    let model_dir = root.join("docs/design/architecture/model");
    for path in crate::walkdir_plain(&model_dir)? {
        if path.extension().is_some_and(|e| e == "c4") {
            let text =
                fs::read_to_string(&path).with_context(|| format!("read {}", path.display()))?;
