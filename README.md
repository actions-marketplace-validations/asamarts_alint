...
- 🧰 **Powerful + extensible.** 84 rule kinds across 13 families, 21 bundled ecosystem rulesets, 12 auto-fix ops, 8 output formats, structured-query rules with full RFC 9535 JSONPath, cross-file relational rules, conditional `when:` gates over per-run facts, and `extends:` composition with SRI-pinned URLs.
...
- **84 rule kinds** across 13 families: existence, content, naming, structured query (RFC 9535 JSONPath over JSON/YAML/TOML/XML), text hygiene, security/unicode, encoding, structure, portable metadata, Unix metadata, git hygiene, cross-file relations, plugin (`command` shellout). Full reference: [`docs/rules.md`](docs/rules.md).
...
- **Official GitHub Action**: [`asamarts/alint@v0.11.1`](https://github.com/asamarts/alint).
...
docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.11.1 check
```
...
Also published: `:<major>.<minor>` (e.g. `:0.10`) and the raw git tag (`:v0.11.1`).
...
- uses: asamarts/alint@v0.11.1
  with:
    version: v0.11.1        # alint release tag (default: latest)
