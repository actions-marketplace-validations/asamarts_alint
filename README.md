...
- **Official GitHub Action**: [`asamarts/alint@v0.14.0`](https://github.com/asamarts/alint).
...
docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.14.0 check
```
...
Also published: `:<major>.<minor>` (e.g. `:0.13`) and the raw git tag (`:v0.14.0`).
...
- uses: asamarts/alint@v0.14.0
  with:
    version: v0.14.0        # alint release tag (default: latest)
